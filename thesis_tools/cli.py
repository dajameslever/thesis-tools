...
def _run_visualize_library_command(args: argparse.Namespace) -> int:
    ...
    if args.write_matrix and stats["total"] > 0:
        ...
        # A living view of the library as it stands right now, overwritten in
        # place — unless it was pointed at output/, where the destination's
        # rule wins and every version is kept. openpyxl saves by path, so the
        # old copy is filed away first rather than through write_view().
        archive_if_kept(matrix_path)
