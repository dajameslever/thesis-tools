...
def score_relevance(query_text: str, paper: Paper) -> float:
    ...
    than its abstract. Deliberately simple/deterministic — no ML dependency —
    ...
    abstract_terms = set(tokenize(paper.abstract or ""))

    title_overlap = len(query_terms & title_terms) / len(query_terms)
    abstract_overlap = len(query_terms & abstract_terms) / len(query_terms) if abstract_terms else 0.0

    score = 0.7 * title_overlap + 0.3 * abstract_overlap
